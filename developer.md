...
3. **Cerrah** — Her sorun için unified diff formatında düzeltme önerir + yan etki riski ve test önerisi üretir.
...
- **🧠 LLM düşünme stream'i, 📋 diff kopyala, 🔎 repo ön-tarama, 🎯 auto-scroll** — polish dokunuşları.

> **MVP kapsamı:**
> - Sadece **Python** desteği. JS/TS sonraki sürüme bırakıldı.
...
| Static Analysis (Python) | `ast` (stdlib) + `tree-sitter-python` | AST düzeyinde precision. **MVP yalnız Python.** |
...
- Cerrah: yok (LLM olmadan diff üretimi anlamsız). UI'da "Düzeltme önerisi (Hibrit veya Derin modda etkin)" mesajı.
...
| Cerrah | 🩹 | **Dr. Cerrah** | "Bozuk kodu onarır" |
...
**Görev:** Her sorun için unified diff formatında somut düzeltme kodu üretmek. Statik modda **çalışmaz** (kullanıcıya "düzeltme önerisi için Hibrit/Derin moda geçin" mesajı).
...
**İşlem:**
1. LLM'e structured prompt: "Bu sorunu nasıl düzeltirsin? Unified diff formatında ver."
2. Diff parser ile doğrula (`unidiff` kütüphanesi) — geçersizse 1 retry, hala geçersizse "manuel öneri" string'i döndür.
3. Yan etki riski + test önerisi + iyileşme tahmini.

**Prompt template (`prompts/surgeon.md`):**
```
Sen bir senior performance engineer'sın. Aşağıdaki kod sorununu düzelt.
...
Düzeltilmiş kodu **unified diff** formatında ver. Sonra:
1. Yan etki riski (1-5)
2. Test önerisi (tek cümle)
3. Tahmini iyileşme (ms ya da %)

JSON döndür: {"diff": "...", "risk_level": int, "test_suggestion": "...", "improvement_estimate": "..."}
```
...
{
  "issue_id": "issue-001",
  "diff": "--- a/src/api/users.py\n+++ b/src/api/users.py\n@@ -45,7 +45,5 @@\n-    for user in users:\n-        posts = Post.query.filter_by(user_id=user.id).all()\n+    user_ids = [u.id for u in users]\n+    posts_by_user = Post.query.filter(Post.user_id.in_(user_ids)).group_by(Post.user_id).all()",
  "risk_level": 2,
  "test_suggestion": "Yanıtın hâlâ tüm post'ları içerdiğini integration test ile doğrula.",
...
**Girdi:** Tüm sorunlar + etki metrikleri + düzeltmeler.
...
## 5. Tespit Edilen Örüntüler (22 örüntü — Python)

Kategoriler: **Performans (8)**, **RAM/Bellek (5)**, **Güvenilirlik (4)**, **Güvenlik (1, ayrı UI bölümü)**, **Kalite (4)**.

> **MVP not:** `MEMORY_LEAK_LISTENER` JS/Node EventEmitter ve DOM odaklı bir örüntü. MVP Python-only olduğundan **MVP sonrası**na bırakıldı. Toplam tespit edilen örüntü: **22**.
...
### 5.2 RAM / Bellek (4 — Python)

> `MEMORY_LEAK_LISTENER` MVP sonrasına bırakıldı (JS odaklı).
...
- Sorun listesi (performans/RAM/güvenilirlik/kalite — kategori başlığıyla gruplu):
  ...
  - "Düzeltmeyi göster" (Statik mod'da disabled).
...
5. `backend/`: `pyproject.toml`, sanal ortam, bağımlılıklar:
   ...
   - `tree-sitter`, `tree-sitter-python` *(JS/TS parserları MVP sonrası)*
   - `unidiff`, `pydantic`, `langgraph`
...
**Yapılacaklar:**
1. `backend/analysis/ast_parser.py` — Python `ast` + tree-sitter-python. *(MVP yalnız Python.)*
2. `backend/analysis/static_rules/base.py` — `StaticRule` arayüzü.
3. **22 Python örüntü dosyasını sırayla yaz**:
...
**Yapılacaklar:**
1. `backend/agents/surgeon.py`.
2. `prompts/surgeon.md` + 3+ few-shot örnek.
3. `backend/analysis/diff_validator.py` (`unidiff`).
4. Test: 5 sorun için diff üret, repo'ya patch uygula, derleniyor mu?

**Çıkış:** >%80 oranında geçerli diff. Retry mekanizması çalışıyor.
...
8. Cerrah bozuk diff → "manuel öneri" string'i, frontend graceful.
...
8. Ucuz kazançlar (§16.7): LLM düşünme stream'i, diff kopyala butonu, repo ön-tarama, auto scroll + highlight.
...
- Diff validator için bozuk diff örnekleri.
...
4. **JS/TS desteği:** **MVP'de yok.** Sadece Python. JS/TS MVP sonrasına bırakıldı.
...
### Minimum (mutlaka)
- Public GitHub linkinden Python repo analizi yapılıyor.
...
- Cerrah en az 1 sorun için geçerli diff üretiyor.
...
| Cerrah bozuk diff | Yüksek | Düşük | Parse + retry + manuel öneri fallback. |
...
#### 16.7.2 📋 Diff Kopyala Butonu
Her düzeltme kartının köşesinde küçük 📋 butonu. `navigator.clipboard.writeText(diff)` + "Kopyalandı ✓" toast (1.5s).
...
- **AI-Generated Test Cases** — Cerrah'ın diff'iyle birlikte unit test üretir. Düzeltme + test = profesyonel ürün.
- **Sağlık Skoru Zaman Çizelgesi** — Repo'nun farklı commitlerindeki skor evrimi grafiği. "Skor son 6 ayda 78 → 62'ye düşmüş."
- **OWASP / Well-Architected Uyum** — Endüstri framework'lerine bağlama; "OWASP Top 10 uyum: 7/10".
- **Otomatik GitHub PR Oluşturma (OAuth)** — Cerrah diff'lerini tek tıkla repo'ya PR olarak aç.
